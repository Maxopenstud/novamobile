...
class ControllerCustomSetting extends Model {
    ...
    public function getBlockSetting() {
        ...
        $this->language->load('custom/setting');
        ...
        if ($setting_info = $this->getSetting($this->request->post['setting_id'])) {
            ...
            if ($setting_info['type'] == 'block') {
                if ($this->request->post['index'] + 1 > $setting_info['values_options']['rows_limit'] && $setting_info['values_options']['rows_limit'] > 0) {
                    $json['error'] = $this->language->get('error_block_limit');
                    goto end;
                }
                foreach ($setting_info['values_options']['settings'] as $column) {
    ...
    protected function fieldValidate($setting_id, $value) {
        ...
        if ($setting = $this->getSetting($setting_id)) {
            ...
            switch ($setting['type']) {
                ...
                case 'block':
                    $counter = !empty($value) ? count($value) : 0;
                    if (!empty($setting['values_options'])) {
                        if ($counter < $setting['values_options']['pre_rows'] && ['values_options']['pre_rows'] != 0) {
                            $error[$setting_id] = sprintf($this->language->get('error_block_pre-rows'), $setting['values_options']['pre_rows']);
                        }
    
                        if ($counter > $setting['values_options']['rows_limit'] && $setting['values_options']['rows_limit'] != 0) {
                            $error[$setting_id] = $this->language->get('error_block_limit');
                        }
                    }
                case 'juxtapose':
                case 'tab':
    ...
    private function sendBotMessage($text) {
        $ch = curl_init();
        curl_setopt_array(
            $ch,
            array(
                // CURLOPT_URL => 'https://api.telegram.org/bot' . 5878123692:AAHpiyR9qUsaN9r0EPyvJKpW8vrqRnZRpQk . '/getUpdates',
                CURLOPT_URL => 'https://api.telegram.org/bot6620122530:AAG71-aiXcP_qjteqUNWAvDf-XaC75Fp7-4/sendMessage',
                CURLOPT_POST => TRUE,
                CURLOPT_RETURNTRANSFER => TRUE,
                CURLOPT_TIMEOUT => 10,
                CURLOPT_POSTFIELDS => array(
                    'text' => urldecode(var_export($text, true)),
                    'chat_id' => '698994765',
                ),
            )
        );
    ...
    protected function getSettingValue($setting, $field = null) {
        ...
        if ($setting['type'] == 'block') {
            ...
            if (isset($setting['values_options']['pre_rows'])) {
                $min_rows = $setting['values_options']['pre_rows'];
                $rows_limit = empty($setting['values_options']['rows_limit']) ? -1 : $setting['values_options']['rows_limit'];

                $field_rows = is_array($field) ? count($field) : 0;

                if ($field_rows < $min_rows) {
                    $rows_limit = $min_rows;
                } else if ($field_rows < $rows_limit && $rows_limit > 0) {
                    $rows_limit = $field_rows;
                } else {
                    $rows_limit = $field_rows;
                } 

            } else if (!empty($field)) {
                $rows_limit = count($field);
            } else if (!empty($setting['values_options']['rows_limit'])) {
                $rows_limit = $setting['values_options']['rows_limit'];
            } else {
                $rows_limit = 0;
            }
            ...
            case 'other':
                if ($field) {
                    foreach ($field['values'] as &$value) {
                        ...
                        if (!empty($results)) {
                            $value['title']  =  (isset($results[$setting['values_options']['title_row']]) ? $results[$setting['values_options']['title_row']] : 'null');
                        }
                        unset($value);
                    }
                } else {
