    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            if (!isset($this->request->get['module_id'])) {
                $module_id = $this->{'model_' . $this->model_place . '_module'}->addModule('how_it_works', $this->request->post);
            } else {
            ...
            if (isset($this->request->post['apply']) && $module_id) {
                $this->response->redirect($this->url->link('extension/module/how_it_works', $this->token_var . '=' . $this->session->data[$this->token_var] . '&module_id=' . $module_id, true));
            } else {
        ...
        if (!isset($this->request->get['module_id'])) {
            $data['breadcrumbs'][] = array(
                'text' => $heading_title,
                'href' => $this->url->link('extension/module/how_it_works', $this->token_var . '=' . $this->session->data[$this->token_var], true)
            );
        } else {
            $data['breadcrumbs'][] = array(
                'text' => $heading_title,
                'href' => $this->url->link('extension/module/how_it_works', $this->token_var . '=' . $this->session->data[$this->token_var] . '&module_id=' . $this->request->get['module_id'], true)
            );
        }
        ...
        if (!isset($this->request->get['module_id'])) {
            $data['action'] = $this->url->link('extension/module/how_it_works', $this->token_var . '=' . $this->session->data[$this->token_var], true);
        } else {
            $data['action'] = $this->url->link('extension/module/how_it_works', $this->token_var . '=' . $this->session->data[$this->token_var] . '&module_id=' . $this->request->get['module_id'], true);
        }
        ...
        $this->response->setOutput($this->load->view('extension/module/how_it_works', $data));
    }

    protected function validate() {
        if (!$this->user->hasPermission('modify', 'extension/module/how_it_works')) {
